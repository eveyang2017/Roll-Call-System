...
export class DataService {
  ...
  // Base Url for API
  private baseUrl: string = (location.hostname === "localhost") ? 'http://193.112.29.23/api/' : 'http://193.112.29.23/api/';
  ...
  public login(username: string, password: string):Promise<JSON> {
    return new Promise((resolve, reject) => {
      ...
      this.http.post(this.baseUrl + 'login/', data).subscribe((result: any) => {
          resolve(result.json());
